...
class Evaluator:
    ...
    def evaluate_single_item(
        self, question, llm_output, answer_range, answer_type, correct_answer
    ):
    ...
    def evaluate(self, data_path):
        ...
        results = [
            self.evaluate_single_item(
                ...
                item["llm_output"],
                (
                    self.MATH_STANDARD_ANSWER_RANGE
                    if item["key_answer_type"] == "math"
                    else item["standard_answer_range"]
                ),
    ...
    def evaluate_single_example(
        ...
        evaluation_result = self.evaluate_single_item(
            question, llm_output, standard_answer_range, key_answer_type, correct_answer
        )
...
def main():
    ...
    parser.add_argument(
        "--run-example", action="store_true", help="Run the example evaluation."
    )
    parser.add_argument(
        "--model-name",
        type=str,
        default="xFinder-qwen1505",
    ...
    parser.add_argument(
        "--model-path-or-url",
        type=str,
        default="IAAR-Shanghai/xFinder-qwen1505",
        help="The path or URL of the model.",
    )
    parser.add_argument(
        "--temperature",
        type=float,
